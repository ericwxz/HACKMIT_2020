...
def DisplayUserProfile(username):
    """returns a json with location, requestIDs, claimIDs, and points"""

def InitializeRequest(username, title, content, timeframe, new_location)name = None):
    """initialize data in database with fields listed above, as well as the hidden fields; if new_location not specified, use user default"""
...
def CompleteRequest(requestID, username):
    """posting user requests to close/complete the request"""
